...
//!	Generated Date	: Sun, 22, May 2022  
	File Path	: StopwatchTest\Microsoft\Timer.c
*********************************************************************/
...
/*#[ ignore */
static void FreeInstance(struct Timer_t* const me);
/*#]*/
...
void Timer_Init(struct Timer_t* const me, RiCTask * p_task) {
    ...
    static const RiCReactive_Vtbl Timer_reactiveVtbl = {
        ...
        rootState_entDef,
        NULL,
        (RiCObjectDestroyMethod) Timer_Destroy,
        NULL,
        NULL,
        NULL,
        (RiCObjectCleanupMethod) Timer_Cleanup,
...
struct Timer_t * Timer_Create(RiCTask * p_task) {
    struct Timer_t* me = (struct Timer_t *) malloc(sizeof(struct Timer_t));
    if(me!=NULL)
        {
            Timer_Init(me, p_task);
        }
...
int Timer_running_IN(const struct Timer_t* const me) {
    return me->rootState_subState == Timer_running;
}
...
int Timer_on_IN(const struct Timer_t* const me) {
    return me->running_subState == Timer_on;
}

int Timer_off_IN(const struct Timer_t* const me) {
    return me->running_subState == Timer_off;
}
